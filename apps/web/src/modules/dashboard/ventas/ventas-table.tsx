...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@camaras/ui/src/components/card"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@camaras/ui/src/components/table"
...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@camaras/ui/src/components/dropdown-menu"
...
const ErrorState = ({ onRetry }: { onRetry?: () => void }) => (
  <div className="flex flex-col items-center justify-center py-12">
    ...
    <h4 className="text-lg font-semibold text-foreground mb-2">Error al cargar ventas</h4>
...
const EmptyState = () => (
  <div className="text-center py-12">
    ...
    <h4 className="text-lg font-semibold text-foreground mb-2">No se encontraron ventas</h4>
    <p className="text-sm text-muted-foreground max-w-sm mx-auto">Aún no hay ventas registradas en el sistema.</p>
  </div>
)
...
const SaleTableRow = ({ sale }: { sale: Sale }) => {
  ...
  return (
    <TableRow>
      <TableCell>
        <div className="font-medium text-foreground">{sale.id.slice(0, 8) + "..."}</div>
      </TableCell>
      <TableCell>
      ...
      <TableCell>
        <Badge variant={getStatusBadgeVariant(sale.status)}>{getStatusText(sale.status)}</Badge>
      </TableCell>
      <TableCell>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-foreground truncate">{sale.buyerName}</div>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Esta acción no se puede deshacer. Se eliminará permanentemente la venta "{sale.id}".
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
...
export function VentasTable() {
  const { data, isLoading, isError } = useSalesPhotographer();
